...
//******************************************************************************************************
CLVEasyItem::CLVEasyItem(uint32 level, bool superitem, bool expanded, float minheight, bool fullLineSelect)
...
void CLVEasyItem::DrawItemColumn(BView *owner, BRect item_column_rect, int32 column_index, bool complete)
{
        ...
        type == CLVColUserText)
    {
        ...
        if (full_line_select == true)        // Added by Ram
        {
        ...
        if(text != NULL)
        {
           ...
           if(!right_justify)
               ...
               // Ram
               draw_point.Set(item_column_rect.left+6.0,item_column_rect.top+text_offset);
           else
           {
               BFont font;
               owner->GetFont(&font);
...
void CLVEasyItem::Update(BView *owner, const BFont *font)
{
    ...
    //font_height FontAttributes;        // Made as data member by Ram
    BFont owner_font;
    owner->GetFont(&owner_font);
...
BRect CLVEasyItem::TruncateText(int32 column_index, float column_width, BFont* font)
{
    //column_width -= 4;           // Ram
    column_width -= 12;
        //Because when I draw the text I start drawing 6 pixels to the right from the column's left edge, and want
        //to stop 6 pixels before the right edge
