...
: BWindow(BRect(50,50,450,450),"Generic test window",B_DOCUMENT_WINDOW,0)
{
    ...
    MyColumnListView->AddColumn(new CLVColumn("Locked at end",237.0,CLV_LOCK_AT_END|CLV_NOT_MOVABLE|
        CLV_SORT_KEYABLE|CLV_MERGE_WITH_RIGHT));
...
void DemoItem::DrawItemColumn(BView* owner, BRect item_column_rect, int32 column_index, bool complete)
{
    ...
    if(column_index == 1)
    {
        ...
        owner->DrawString(fText[column_index-2],
...
{
    char* Text1 = ((DemoItem*)a_Item1)->fText[KeyColumn-2];
    char* Text2 = ((DemoItem*)a_Item2)->fText[KeyColumn-2];
