 ...
 * Modified code: Ramshankar (Ram), Jeremy Freisner (jaf)
 *
...
uint8 CLVRightArrowData[132] =
{
	...
	0xFF, 0xFF, 0xFF, 0x00, 0xCC, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
	0xFF, 0xFF, 0xFF, 0x00, 0xCC, 0xCC, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
	0xFF, 0xFF, 0xFF, 0x00, 0xCC, 0xCC, 0xCC, 0x00, 0xFF, 0xFF, 0xFF, 0xFF,
	0xFF, 0xFF, 0xFF, 0x00, 0xCC, 0xCC, 0xCC, 0xCC, 0x00, 0xFF, 0xFF, 0xFF,
	0xFF, 0xFF, 0xFF, 0x00, 0xCC, 0xCC, 0xCC, 0x00, 0xFF, 0xFF, 0xFF, 0xFF,
	0xFF, 0xFF, 0xFF, 0x00, 0xCC, 0xCC, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
	0xFF, 0xFF, 0xFF, 0x00, 0xCC, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
	0xFF, 0xFF, 0xFF, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
...
uint8 CLVDownArrowData[132] =
{
	...
	0xFF, 0x00, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0x00, 0xFF, 0xFF,
	0xFF, 0xFF, 0x00, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0x00, 0xFF, 0xFF, 0xFF,
	0xFF, 0xFF, 0xFF, 0x00, 0xBB, 0xBB, 0xBB, 0x00, 0xFF, 0xFF, 0xFF, 0xFF,
	0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0xBB, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
...
fDownArrow(BRect(0.0,0.0,10.0,10.0),B_COLOR_8_BIT,CLVDownArrowData,false,false)
{
	...
	*ContainerView = fScrollView;
	...
	fSelectedItemForeColorWindowActive = Black;
	fSelectedItemForeColorWindowInactive = Black;
	fWindowActive = false;
}
...
// Commented out by Ram -- we no longer want CLV to check before EACH operation
// if our window is locked or not, if not it is going to crash so why have a check?
// Let it crash anyway
...
void ColumnListView::SetSortKey(int32 ColumnIndex, bool sortNow)
{
	//AssertWindowLocked();
	CLVColumn* Column;
	if(ColumnIndex >= 0)
...
bool ColumnListView::AddUnderFast(BListItem* a_item, BListItem* a_superitem)
{
	//Get the CLVListItems
	CLVListItem* item = cast_as(a_item,CLVListItem);
	CLVListItem* superitem cast_as(a_superitem,CLVListItem);
	...
	{
		CLVListItem* Temp = (CLVListItem*)fFullItemList.ItemAt(ItemPos);
		if(!Temp || Temp->fOutlineLevel <= SuperItemLevel)
			break;
...
bool ColumnListView::AddItemFastHierarchical(BListItem* a_item)
{
	//Get the CLVListItems
	CLVListItem* item = cast_as (a_item,CLVListItem);
	return AddItemPrivate (item,fFullItemList.CountItems());
...
{
//	AssertWindowLocked();	-- Commented out by Ram
...
void ColumnListView::SortListArray (CLVListItem** SortArray, int32 NumberOfItems)
{
	...
	//	meaning when all items in the column are same, no items will be re-ordered. -- Ram
	bool canSort = false;
	CLVColumn* Column = (CLVColumn*)fSortKeyList.ItemAt(0);
	if (NumberOfItems > 1)
		for (int32 i = 0; i < NumberOfItems; i++)
			if (fCompare (FullListItemAt(i), FullListItemAt(i+1), &fColumnList,fColumnList.IndexOf(Column)) != 0)
			{
				canSort = true;
				break;
			}

	// We use quicksort now, instead of bubblesort -- Ram
	if (canSort == true)
		QuickSort (SortArray, NumberOfItems);

	Window()->Unlock();
}

void ColumnListView::QuickSort (CLVListItem **SortArray, int32 n)
{
	...
	for (int32 Counter1 = 1; Counter1 < n; ++Counter1)
	{
