    ...
    if(Buttons == B_PRIMARY_MOUSE_BUTTON)
    {
        ...
        if(GrabbedResizeTab)
        {
            ...
            if(!(ThisColumn->fFlags & CLV_NOT_RESIZABLE))
            {
                ...
                Invalidate(BRect(fColumnClicked->fColumnEnd-1.0,ViewBounds.top,ThisColumn->fColumnEnd,
                    ViewBounds.bottom));    // -1.0 Ram -- because we have a thick border now
                    ...
                    && Point.x >= previousPoint.x - 1 && Point.y <= previousPoint.x + 1))
            {
                ...
                for (int32 i = 0; i < fParent->CountItems(); i++)
                {
                    ...
                    const char *text = ((CLVEasyItem*)listItem)->GetColumnContentText (columnIndex);
                    float indent = 12.0;
                    if (columnIndex == 2)
...
void CLVColumnLabelView::MouseUp(BPoint Point)
{
    if(fColumnClicked == NULL)
        return;
    BRect ViewBounds = Bounds();
        ...
        if(ColumnFlags&CLV_SORT_KEYABLE)
        {
            ...
            if(!(Modifiers&B_SHIFT_KEY))
            {
                ...
                if(fParent->fSortKeyList.ItemAt(0) == fColumnClicked)
                    ...
                    fParent->SetSortKey(fParent->fColumnList.IndexOf(fColumnClicked), true);
            }
            else
            {
